...
def main():
    ...
    spark, log, config = start_spark(
        app_name='my_etl_job',
        files=[f'{root_dir}/configs/etl_config.json'])
...
def extract_data(spark):
    ...
    df = (
        spark
        .read
        .parquet(f'{root_dir}/tests/test_data/employees'))
    ...
    df_transformed = (
        ...
        .select(
            ...
            concat_ws(
                ...
                col('first_name'),
                col('second_name')).alias('name'),
               (col('floor') *steps).alias('steps_to_desk')))
...
def create_test_data(spark, config):
    ...
    (df
     .coalesce(1)
     .write
     .parquet(f'{root_dir}/tests/test_data/employees', mode='overwrite'))
    ...
    (df_tf
     .coalesce(1)
     .write
     .parquet(f'{root_dir}/tests/test_data/employees_report', mode='overwrite'))
